...
The strings are generated using multiprocessing. Multiple jobs are created, each in charge a fixed number of strings, for a total of at least 4MB of strings (in the extreme case where all generated strings have the shortest allowed length). As all the strings are conctenated, the 37 Bytes overhead per string only exists once per 4MB, and thus is neglected.
...
In order to keep the sorting algorithm simple, python sorted() algorithm is used. This algorithm while efficient and fast, loads th file into the RAM, so cannot be used flawlessely for very big databases.
Multiprocessing is again used, similarly for the previous program - each expensiveFunc() is a separate job with an additional job for writting. This enables parallel processing of the strings, which will be very advantageuse if expensiveFunc() was a heavy function.
